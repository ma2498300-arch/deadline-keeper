...
function DashboardApp({ onLogout }: { onLogout: () => void }) {
  ...
  const visibleProjects = useMemo(() => projects.filter(project => filter === 'all' || project.status === filter), [projects, filter])
  ...
  function saveProject(form: FormState) {
    const now = new Date().toISOString()
    const current = editing
    if (current) {
      updateProject(current.id, { ...form, status: new Date(form.deadline) < new Date() ? 'overdue' : current.status === 'completed' ? 'completed' : 'active' })
      playTone('save', muted)
    } else {
      setProjects(existing => [{ ...form, id: crypto.randomUUID(), status: 'active', createdAt: now, updatedAt: now, notifiedOneDayLeft: false, notifiedOverdue: false }, ...existing])
      playTone('save', muted)
    }
  ...
  return <div className="app-shell font-body" dir={settings.language === 'ar' ? 'rtl' : 'ltr'} style={{ backgroundColor: background?.color || '#2E2924' }}>
    ...
    <main className="content-width main-content">
      ...
      <div className="toolbar">
        ...
        <span className="breathing-label">{visibleProjects.length ? `${visibleProjects.length} ${tr(language, 'thingsInView')}` : tr(language, 'quietPage')}</span>
      </div>

      {visibleProjects.length ? <motion.div layout className="project-grid">{visibleProjects.map(project => <ProjectCard key={project.id} project={project} language={language} onEdit={openEdit} onComplete={toggleComplete} />)}</motion.div> : <EmptyState language={language} onCreate={openCreate} />}</> : <TimelineView projects={projects} language={language} onEdit={openEdit} />}
    </main>

    <footer className="footer content-width"><span>{tr(language, 'made')}</span><span>{tr(language, 'device')}</span></footer>
    <AnimatePresence>{(isCreating || editing) && <ProjectModal initial={editing} language={language} onClose={() => { setEditing(null); setIsCreating(false) }} onSave={saveProject} onRequestNotifications={requestNotifications} />}</AnimatePresence>
    <AnimatePresence>{settingsOpen && <SettingsPanel settings={settings} language={language} onChange={setSettings} onClose={() => setSettingsOpen(false)} />}</AnimatePresence>
    <AnimatePresence>{notice && <motion.div className="toast" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }} transition={{ type: 'spring', bounce: 0.35 }} onAnimationComplete={() => setTimeout(() => setNotice(null), 3800)}>{notice}<button onClick={() => setNotice(null)} aria-label={tr(language, 'dismiss')}>×</button></motion.div>}</AnimatePresence>
  </div>
}
...
function SettingsPanel({ settings, language, onChange, onClose }: { settings: UserSettings; language: UserSettings['language']; onChange: (settings: UserSettings) => void; onClose: () => void }) {
  ...
  async function handleMedia(file?: File) { if (!file) return; const type = file.type.startsWith('video/') ? 'video' : 'image'; const reader = new FileReader(); reader.onload = () => patch({ appBackground: { type, url: String(reader.result), color: settings.appBackground?.color || '#2E2924' } }); reader.readAsDataURL(file) }
...
function ProjectModal({ initial, language, onClose, onSave, onRequestNotifications }: { initial: Project | null; language: UserSettings['language']; onClose: () => void; onSave: (form: FormState) => void; onRequestNotifications: () => void }) {
...
async function ensureUserSettings(userId: string) {
  const { data } = await supabase.from('user_settings').select('id').eq('user_id', userId).maybeSingle()
...
function App() {
  ...
  return <DashboardApp onLogout={() => { void supabase.auth.signOut() }} />
}
